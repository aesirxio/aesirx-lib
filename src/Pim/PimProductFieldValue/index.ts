...
import { ProductFieldValueItemModel, ProductFieldValueModel } from './Model';
import ProductFieldValueRouter from './Route';
 ...
 * API Service
 */
class ProductFieldValueApiService {
  route: any = null;

  constructor() {
    this.route = new ProductFieldValueRouter();
  }
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new ProductFieldValueModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new ProductFieldValueItemModel(data);
      }
      if (results) {
...
export { ProductFieldValueApiService };
