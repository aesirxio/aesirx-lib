...
import { CountryItemModel, CountryModel } from './Model';
import CountryRouter from './Route';
...
class CountryApiService {
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new CountryModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new CountryItemModel(data);
      }
      if (results) {
