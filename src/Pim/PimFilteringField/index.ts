...
import { FilteringFieldItemModel, FilteringFieldModel } from './Model';
import FilteringFieldRouter from './Route';
 ...
 * API Service
 */
class FilteringFieldApiService {
  route: any = null;

  constructor() {
    this.route = new FilteringFieldRouter();
  }
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new FilteringFieldModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new FilteringFieldItemModel(data);
      }
      if (results) {
...
export { FilteringFieldApiService };
