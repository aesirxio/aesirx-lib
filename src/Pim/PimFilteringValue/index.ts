...
import { FilteringValueItemModel, FilteringValueModel } from './Model';
import FilteringValueRouter from './Route';
 ...
 * API Service
 */
class FilteringValueApiService {
  route: any = null;

  constructor() {
    this.route = new FilteringValueRouter();
  }
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new FilteringValueModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new FilteringValueItemModel(data);
      }
      if (results) {
...
export { FilteringValueApiService };
