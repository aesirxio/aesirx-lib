...
import { TypeItemModel, TypeModel } from './Model';
import TypeRouter from './Route';
...
class TypeApiService {
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new TypeModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new TypeItemModel(data);
      }
      if (results) {
