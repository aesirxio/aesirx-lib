...
import { ShippingMethodItemModel, ShippingMethodModel } from './Model';
import ShippingMethodRouter from './Route';
...
class ShippingMethodApiService {
  ...
  constructor() {
    this.route = new ShippingMethodRouter();
  }
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new ShippingMethodModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new ShippingMethodItemModel(data);
      }
      if (results) {
...
export { ShippingMethodApiService };
