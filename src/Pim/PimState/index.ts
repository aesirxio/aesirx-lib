...
import { StateItemModel, StateModel } from './Model';
import StateRouter from './Route';
...
class StateApiService {
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new StateModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new StateItemModel(data);
      }
      if (results) {
