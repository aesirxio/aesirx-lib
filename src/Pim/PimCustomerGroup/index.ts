...
import { CustomerGroupItemModel, CustomerGroupModel } from './Model';
import CustomerGroupRouter from './Route';
...
class CustomerGroupApiService {
  ...
  constructor() {
    this.route = new CustomerGroupRouter();
  }
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new CustomerGroupModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new CustomerGroupItemModel(data);
      }
      if (results) {
...
export { CustomerGroupApiService };
