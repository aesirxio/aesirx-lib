...
import { CityItemModel, CityModel } from './Model';
import CityRouter from './Route';
...
class CityApiService {
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new CityModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new CityItemModel(data);
      }
      if (results) {
