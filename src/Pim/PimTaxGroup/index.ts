...
import { TaxGroupItemModel, TaxGroupModel } from './Model';
import TaxGroupRouter from './Route';
...
class TaxGroupApiService {
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new TaxGroupModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new TaxGroupItemModel(data);
      }
      if (results) {
