...
import { ShippingRateItemModel, ShippingRateModel } from './Model';
import ShippingRateRouter from './Route';
...
class ShippingRateApiService {
  ...
  constructor() {
    this.route = new ShippingRateRouter();
  }
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new ShippingRateModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new ShippingRateItemModel(data);
      }
      if (results) {
