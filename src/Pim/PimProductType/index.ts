...
import { ProductTypeItemModel, ProductTypeModel } from './Model';
import ProductTypeRouter from './Route';
 ...
 * API Service
 */
class ProductTypeApiService {
  route: any = null;

  constructor() {
    this.route = new ProductTypeRouter();
  }
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new ProductTypeModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new ProductTypeItemModel(data);
      }
      if (results) {
