...
import { RegionItemModel, RegionModel } from './Model';
import RegionRouter from './Route';
...
class RegionApiService {
  ...
  getList = async (filter: any) => {
    try {
      ...
      const roleItems = new RegionModel(data);
  ...
  getDetail = async (id = 0) => {
    try {
      ...
      let results = null;
      if (data) {
        results = new RegionItemModel(data);
      }
      if (results) {
